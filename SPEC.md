...
- 2026-09-13 22:35 (Manila) SAVEPOINT. Completed since 21:30: admin format glyphs (Zoom camera gold / self-record dot green) on day tiles, drawer cards, search results + legend; page widened to 1520px and, on ≥1200px screens, an open drawer pushes content aside with the wrap stretching to the drawer edge; calendar columns forced equal (minmax(0,1fr)) after long names squeezed Friday; day lines vertically centred. Prod admin deployment @8. Live intake form tested end-to-end in headless Chrome (self-record, 2026-09-22): success card shown, row landed, availability updated — test row "TEST SUBMISSION - delete me" still to be deleted from the sheet. Ellana's admin access resolved: multi-account browser → use the domain URL form https://script.google.com/a/macros/fellowacademy.com.au/s/<admin id>/exec?page=admin or a single-account profile. Commits 683d6c7, 7e56ce4. Tests 19/19. Blocked: Jeraisy to delete the test row (22 Sep shows full until then) and to copy stragglers from the old sheet if any. Next: routine use; deploys via `bash apps-script/deploy.sh prod all`.
